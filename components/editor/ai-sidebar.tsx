...
export function AiSidebar({ isOpen, onClose, roomId, projectId }: AiSidebarProps) {
  ...
  const handleRunTerminal = useCallback(
    (status: string, output: unknown) => {
      ...
      const content = isSuccess
        ...
        : "Architect AI encountered an error. Please try again."
  ...
  const handleSend = useCallback(async () => {
    ...
    createFeedMessage(FEED_ID, {
      text: "Architect AI is analyzing your request…",
      status: "start",
    }).catch(() => {})

    setStatusText("Architect AI is analyzing your request…")
    ...
    try {
      ...
      createFeedMessage(CHAT_FEED_ID, {
        sender: "Architect AI",
        role: "assistant",
        content: "Failed to reach Architect AI. Please try again.",
        timestamp: new Date().toISOString(),
      }).catch(() => {})

      createFeedMessage(FEED_ID, {
        text: "Architect AI encountered an error.",
        status: "error",
      }).catch(() => {})
    ...
    >
      ...
      <div className="flex shrink-0 items-center gap-3 border-b border-border-default px-5 py-4">
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="text-xs text-text-muted">Collaborate with Architect AI</p>
        </div>
        {isLoading && (
